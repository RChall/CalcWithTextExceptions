...
public class Main
{
    ...
    private static String toRoman (int numArabian) {
        ...
        if (numArabian >= 10 || numArabian <= -10){
            return null;
        }
...
// Цикл определения знака, действия
        ...
        if ( what.equals("deystvie")) {
            System.out.println("ОШИБКА!!! Недопустимая арифмитическая операция");
        }
        i=0;
...
// Счетчик введенных римских чисел для вывода
        ...
        if ( arabOrRome == 2 && summa < 1) {
            arabOrRome +=100500;
            System.out.println("ОШИБКА!!! Римское число не может быть меньше единицы" ) ;
        }
        if ( frstNumInt > 10 || scndNumInt > 10 || frstNumInt < -10 || scndNumInt < -10 ) {
            arabOrRome +=100500;
            System.out.println("ОШИБКА!!! Введены числа вне допустимых значений" ) ;
        }

// Вывод
        switch (arabOrRome) {
            case 2 -> System.out.println("Результат: " + toRoman(summa));
            case 1 -> System.out.println("ОШИБКА!!! Невозможно использовать одновременно разные системы счисления");
